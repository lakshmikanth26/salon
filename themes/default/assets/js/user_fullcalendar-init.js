  ...
  "use strict";
  	...
  	document.addEventListener('DOMContentLoaded', function() {
		...
		target.fullCalendar({
			...
			columnHeader:true,
			events: {
				url: site.base_url+'dashboard/get_appointments',
				error: function() {
					$('#script-warning').show();
				}
			},
			...
			select: function (start, end, allDay) {
	          ...
	          if(start.isBefore(moment(), 'day')) {
				...
				if(event_start_date){
					$.get(site.base_url+'dashboard/get_staffs',
				    {
				...
				$.get(site.base_url+'dashboard/get_services',
			    {
			        event_start_date: event_start_date
			    },
				...
				$.get(site.base_url+'dashboard/get_warehouses',
			    {
			        event_start_date: event_start_date
			    },
