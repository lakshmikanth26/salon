...
var old_po_ref;
$('#slpo_ref').focus(function () {
	old_po_ref = $(this).val();
}).change(function (e) {
	var new_po_ref = $(this).val() ? parseFloat($(this).val()) : 0;
	if (!is_numeric($(this).val())) {
		$(this).val(old_po_ref);
		bootbox.alert(lang.unexpected_value);
		return;
	} else {
		localStorage.setItem('slpo_ref', new_po_ref);
		$('#slpo_ref').val(new_po_ref);
	}
});
if (slpo_ref = localStorage.getItem('slpo_ref')) {
	$('#slpo_ref').val(slpo_ref);
}

var old_supplier_ref;
$('#slsupplier_ref').focus(function () {
	old_supplier_ref = $(this).val();
}).change(function (e) {
	var new_supplier_ref = $(this).val() ? parseFloat($(this).val()) : 0;
	if (!is_numeric($(this).val())) {
		$(this).val(old_supplier_ref);
		bootbox.alert(lang.unexpected_value);
		return;
	} else {
		localStorage.setItem('slsupplier_ref', new_supplier_ref);
		$('#slsupplier_ref').val(new_supplier_ref);
	}
});
if (slsupplier_ref = localStorage.getItem('slsupplier_ref')) {
	$('#slsupplier_ref').val(slsupplier_ref);
}

var old_dispatched_throw;
$('#sldispatched_throw').focus(function () {
	old_dispatched_throw = $(this).val();
}).change(function (e) {
	var new_dispatched_throw = $(this).val() ? parseFloat($(this).val()) : 0;
	if (!is_numeric($(this).val())) {
		$(this).val(old_dispatched_throw);
		bootbox.alert(lang.unexpected_value);
		return;
	} else {
		localStorage.setItem('sldispatched_throw', new_dispatched_throw);
		$('#sldispatched_throw').val(new_dispatched_throw);
	}
});
if (sldispatched_throw = localStorage.getItem('sldispatched_throw')) {
	$('#sldispatched_throw').val(sldispatched_throw);
}
	...
	$('#reset').click(function (e) {
		bootbox.confirm(lang.r_u_sure, function (result) {
			if (result) {
				...
				if (localStorage.getItem('slpo_ref')) {
					localStorage.removeItem('slpo_ref');
				}
				if (localStorage.getItem('slsupplier_ref')) {
					localStorage.removeItem('slsupplier_ref');
				}
				if (localStorage.getItem('sldispatched_throw')) {
					localStorage.removeItem('sldispatched_throw');
				}
...
function loadItems() {
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_hsn_sac = item.row.hsn_sac, item_price = item.row.price, item_qty = item.row.qty, item_per_type = item.row.per_type, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
