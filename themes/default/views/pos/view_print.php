...
<div id="wrapper">
    ...
    <div id="receipt-data">
        ...
        <div class="text-center">
            ...
            echo "<p>" .lang("customer") . ": " . $inv->customer . "<span class='text-right' style='float:right;'>Bill No: ".$inv->reference_no."</span></p>";
            echo "<p>" .lang("phone") . ": " . $customer->phone . "<span class='text-right' style='float:right;'>".lang("date").": ".$this->sma->hrld($inv->date)."</span></p>";
            if($customer_group->id == 5){
            echo lang("customer_group") . ": " . $customer->customer_group_name." <br>";
            }
            echo "</p>";
            ?>
            ...
            <thead id="y-border">
                ...
                foreach ($rows as $row) {
                    ...
                    if (isset($tax_summary[$row->tax_code])) {
                        ...
                        $tax_summary[$row->tax_code]['sname'] = $row->tax_rate / 2;

                        $tax_summary[$row->tax_code]['cname'] = $row->tax_rate / 2;
                    ...
                    echo '<td colspan="2">#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') . '</td>';
                ...
                echo '<tr><th colspan="5">' . lang("tax") . '</th><th class="text-right">' . $this->sma->formatMoney($inv->product_tax) . '</th></tr>';
                 } ?>
                <?php

                if ($inv->order_tax != 0) {

                    echo '<tr><th colspan="5">' . lang("tax") . '</th><th class="text-right">' . $this->sma->formatMoney($inv->order_tax) . '</th></tr>';

                }

                if ($inv->order_discount != 0) {
                    if(!empty($discount->name)){
                        echo '<tr><th colspan="5">' . lang("Offer_discount") . '('.$discount->name. ')</th><th class="text-right">'.$this->sma->formatMoney($inv->order_discount) . '</th></tr>';

                    }else{
                        echo '<tr><th colspan="5">' . lang("Offer_discount") . '</th><th class="text-right">'.$this->sma->formatMoney($inv->order_discount) . '</th></tr>';
                ...
                if ($inv->paid < $inv->grand_total) { ?>
                    ...
                    <tr>
                        ...
                        <th colspan="5"><?= lang("paid_amount"); ?></th>
                    ...
                    <tr>
                        ...
                        <th colspan="5"><?= lang("due_amount"); ?></th>
                ...
                <?php } ?>
                <?php if($customer_group->id == 5){ ?>
                    <tr style="border-top: 2px solid #000;">

                        <th colspan="5"><?= lang("Non_Member_Price & without Discount"); ?></th>

                        <th class="text-right"><?= $this->sma->formatMoney($inv->non_member_price+$inv->order_discount); ?></th>

                    </tr>
                    <tr>

                        <th colspan="5"><?= lang("Your_Save"); ?></th>

                        <th class="text-right"><?= $this->sma->formatMoney(($inv->non_member_price - $inv->grand_total)+$inv->order_discount); ?></th>
            ...
            if ($payments) {
                ...
                foreach ($payments as $payment) {
                    ...
                    if ($payment->paid_by == 'cash' && $payment->pos_paid) {
                        ...
                        echo '<tr><td colspan="5">' . lang("paid_by") . '</td><td class="text-right">' . lang($payment->paid_by) . '</td></tr>';

                        echo '<tr><td colspan="5">' . lang("amount") . '</td><td class="text-right">' . $this->sma->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td></tr>';

                        echo '<tr><td colspan="5">' . lang("change") . '</td><td class="text-right">' . ($payment->pos_balance > 0 ? $this->sma->formatMoney($payment->pos_balance) : 0) . '</td></tr>';
                    ...
                    if (($payment->paid_by == 'CC' || $payment->paid_by == 'ppp' || $payment->paid_by == 'stripe') && $payment->cc_no) {
                        ...
                        echo '<tr><td colspan="5">' . lang("amount") . '</td><td class="text-right">' . $this->sma->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td></tr>';

                        echo '<tr><td colspan="5">' . lang("no") . '</td><td class="text-right">' . 'xxxx xxxx xxxx ' . substr($payment->cc_no, -4) . '</td></tr>';

                        echo '<tr><td colspan="5">' . lang("name") . '</td><td class="text-right">' . $payment->cc_holder . '</td></tr>';
                    ...
                    if ($payment->paid_by == 'Cheque' && $payment->cheque_no) {
                        ...
                        echo '<tr><td colspan="5">' . lang("cheque_no") . '</td><td class="text-right">' . $payment->cheque_no . '</td></tr>';
                    ...
                    if ($payment->paid_by == 'gift_card' && $payment->pos_paid) {
                        ...
                        echo '<tr><td colspan="5">' . lang("no") . '</td><td class="text-right">' . $payment->cc_no . '</td></tr>';

                        echo '<tr><td colspan="5">' . lang("amount") . '</td><td class="text-right">' . $this->sma->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td></tr>';

                        echo '<tr><td colspan="5">' . lang("balance") . '</td><td class="text-right">' . ($payment->pos_balance > 0 ? $this->sma->formatMoney($payment->pos_balance) : 0) . '</td></tr>';
                    ...
                    if ($payment->paid_by == 'other' && $payment->amount) {
                        ...
                        echo $payment->note ? '</tr><td colspan="2">' . lang("payment_note") . '</td><td class="text-right">' . $payment->note . '</td></tr>' : '';
            ...
            if ($Settings->invoice_view == 1) {
                ...
                // if ($inv->invoice_type == 1) {
                    ...
                    echo '<table class="table table-condensed"><thead><tr><th>' . lang('name') . '</th><th>' . lang('SGST') . '</th><th>' . lang('CGST') . '</th><th>' . lang('qty') . '</th><th class="text-right">' . lang('tax_amt') . '</th></tr></td><tbody>';

                    foreach ($tax_summary as $summary) {

                        echo '<tr><td>' . $summary['name'] . '</td><td>' . $summary['sname'] . '% ('.$this->sma->formatNumber(($summary['tax']) / 2).')</td><td>' . $summary['cname'] . '% ('.$this->sma->formatNumber(($summary['tax']) / 2).')</td><td class="text-center">' . $this->sma->formatNumber($summary['items']) . '</td><td class="text-right">' . $this->sma->formatMoney($summary['tax']) . '</td></tr>';
        ...
        <div class="text-center">
                            ...
                            <a href="<?= site_url('pos/view/' . $inv->id) ?>" class="tip btn btn-primary no-print" title="<?= lang('print') ?>">
