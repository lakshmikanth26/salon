                ...
                <table class="table table-bordered table-hover table-striped print-table order-table">
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="width: 80px; text-align:center; vertical-align:middle;"><?= $row->hsn_sac; ?></td>
                            <td style="width: 80px; text-align:center; vertical-align:middle;"><?= floatval($row->quantity).' '.$row->per_type_name; ?></td>
                            <td style="text-align:right; width:100px;"><?= $this->sma->formatMoney($row->net_unit_price); ?></td>
                            <?php
                            if ($Settings->tax1) {
                                echo '<td style="width: 100px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_code ? '<small>('.floatval($row->tax_rate).'%)</small>' : '') . ' ' . $this->sma->formatMoney($row->item_tax) . '</td>';
                            }
                            if ($Settings->product_discount && $inv->product_discount != 0) {
