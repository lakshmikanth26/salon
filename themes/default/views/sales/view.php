                    ...
                    <table class="table table-bordered table-hover table-striped print-table order-table">
                        ...
                        foreach ($rows as $row):
                            ...
                            <tr>
                                ...
                                <td style="width: 100px; text-align:center; vertical-align:middle;"><?= $row->hsn_sac; ?></td>
                                <td style="width: 100px; text-align:center; vertical-align:middle;"><?= intval($row->quantity).' '.$row->per_type_name; ?></td>
                                <td style="text-align:right; width:120px; padding-right:10px;"><?= $this->sma->formatMoney($row->net_unit_price); ?></td>
                                <?php
                                if ($Settings->tax1) {
                                    echo '<td style="width: 120px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_code ? '<small>('.floatval($row->tax_rate).'%)</small>' : '') . ' ' . $this->sma->formatMoney($row->item_tax) . '</td>';
                                }
                                if ($Settings->product_discount && $inv->product_discount != 0) {
                    ...
                    <div class="col-xs-6">
                        ...
                        if ($Settings->invoice_view == 1) {
                            if (!empty($tax_summary)) {
                                ...
                                echo '<table class="table table-bordered table-condensed"><thead><tr><th>' . lang('name') . '</th><th>' . lang('sgst') . '</th><th>' . lang('cgst') . '</th><th>' . lang('qty') . '</th><th>' . lang('tax_excl') . '</th><th>' . lang('tax_amt') . '</th></tr></td><tbody>';
                                foreach ($tax_summary as $summary) {
                                    echo '<tr><td>' . $summary['name'] . '</td><td class="text-center">(' .floatval($summary['rate']/2).'%)'. $this->sma->formatMoney($summary['tax']/2) . '</td><td class="text-center">(' .floatval($summary['rate']/2).'%)'. $this->sma->formatMoney($summary['tax']/2) . '</td><td class="text-center">' . intval($summary['items']) . '</td><td class="text-right">' . $this->sma->formatMoney($summary['amt']) . '</td><td class="text-right">' . $this->sma->formatMoney($summary['tax']) . '</td></tr>';
                                }
                                echo '</tbody></tfoot>';
                                echo '<tr><th colspan="5" class="text-right">' . lang('total_tax_amount') . '</th><th class="text-right">' . $this->sma->formatMoney($inv->product_tax) . '</th></tr>';
                                echo '</tfoot></table>';
                            }
                        }
                        ?>
