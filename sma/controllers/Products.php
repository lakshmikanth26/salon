...
class Products extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        ...
        if ($this->Owner || $this->Admin) {
            ...
            $this->data['categories'] = $this->site->getAllCategoriesOnlyProduct();
    ...
    function tax_rate($tax_rate = NULL, $rate=NULL){

        $tax_rate = $_GET['t'];
        $rate = $_GET['r'];
        $this->db->select('rate');
        $this->db->where('id', $tax_rate);
        $this->db->from('tax_rates');
        $q = $this->db->get();
        $t_rate = $q->row();
        $trate = $t_rate->rate;
        
        $cost = $rate + ($rate * ($trate/100));
        echo $cost;
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'rate' => $this->sma->formatDecimal($this->input->post('rate')),
                'price' => $this->sma->formatDecimal($this->input->post('price')),
                'member_price' => $this->sma->formatDecimal($this->input->post('member_price')),
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('code', 'name', 'category_code', 'unit', 'cost', 'member_price', 'price', 'alert_quantity','hsn_sac','per_type', 'tax_rate', 'tax_method', 'track_quantity', 'type', 'subcategory_code', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6');
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if ($catd = $this->products_model->getCategoryByCode(trim($csv_pr['category_code']))) {
                        ...
                        $pr_mprice[] = trim($csv_pr['member_price']);
                        $pr_aq[] = trim($csv_pr['alert_quantity']);
                        $pr_tq[] = trim($csv_pr['track_quantity']);
                        $pr_type[] = trim($csv_pr['type']);
                        $tax_details = $this->products_model->getTaxRateByCode(trim($csv_pr['tax_rate']));
            ...
            $ikeys = array('code', 'name', 'category_id', 'unit', 'cost', 'member_price', 'price', 'alert_quantity','hsn_sac','per_type', 'tax_rate', 'tax_method', 'track_quantity', 'type', 'subcategory_id', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6');

            $items = array();
            foreach (array_map(null, $pr_code, $pr_name, $pr_cat, $pr_unit, $pr_cost, $pr_mprice, $pr_price, $pr_aq,$pr_hsn, $pr_per, $pr_tax, $tax_method, $pr_tq, $pr_type, $pr_subcat, $pr_variants, $cf1, $cf2, $cf3, $cf4, $cf5, $cf6) as $ikey => $value) {
