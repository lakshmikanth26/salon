...
//generate pdf and force to download
    ...
    function getExpenses()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('expenses') . ".id as id, date, reference, company_name, gst_no, cost, tax_amount, amount, note, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as user, attachment", FALSE)
            ->from('expenses')
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('gst_no'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $expense->company_name);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $expense->gst_no);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->sma->formatMoney($expense->cost));
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->sma->formatMoney($expense->tax_amount));
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $this->sma->formatMoney($expense->amount));
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $expense->note);
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $user->first_name . ' ' . $user->last_name);
                        $row++;
                    }
