...
class Customers extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->data['gender_options'] = array(
            'male'   => 'Male',
            'female' => 'Female',
            'other'  => 'Other',
        );
    ...
    function getCustomerCount() {
        ...
        $memberCount = $this->companies_model->getMembershipCount('MEMBER');
        $nonMemberCount = $this->companies_model->getMembershipCount('NON MEMBER');
        $totalRecords = $this->companies_model->getTotalRecords();
    ...
    function getCustomer($id = NULL)
    {
        ...
        $couponrow = $this->companies_model->getCoupons($id);

        $walletrow = $this->companies_model->getWallets($id);
    ...
    function customer_all_actions()
    {
        if (!$this->Owner) {
            $this->session->set_flashdata('warning', lang('access_denied'));
            redirect($_SERVER["HTTP_REFERER"]);
        }
        if ($this->input->post('form_action') == 'export_all_excel' || $this->input->post('form_action') == 'export_all_pdf') {
            ...
            $this->excel->getActiveSheet()->SetCellValue('O1', lang('ccf5'));
            $this->excel->getActiveSheet()->SetCellValue('P1', lang('ccf6'));
            $cutomers = $this->companies_model->getAllCustomerCompanies();
            ...
            $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $filename = 'customers_' . date('Y_m_d_H_i_s');
            if ($this->input->post('form_action') == 'export_all_pdf') {
                $styleArray = array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)));
                $this->excel->getDefaultStyle()->applyFromArray($styleArray);
                $this->excel->getActiveSheet()->getPageSetup()->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
                require_once(APPPATH . "third_party" . DIRECTORY_SEPARATOR . "MPDF" . DIRECTORY_SEPARATOR . "mpdf.php");
            ...
            if ($this->input->post('form_action') == 'export_all_excel') {
                header('Content-Type: application/vnd.ms-excel');
                header('Content-Disposition: attachment;filename="' . $filename . '.xls"');
                header('Cache-Control: max-age=0');
