...
class Sales extends MY_Controller
{
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("id, date, reference_no, biller, customer,staff, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status")
                ->from('sales')
    ...
    function view($id = NULL)
    {
        ...
        $this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
        $this->data['staff'] = $this->site->getStaffByID($inv->staff_id);
        $this->data['bank'] = $this->site->getBankDetailByID($inv->bank_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : NULL;
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
    ...
    function add($quote_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $bank_id = $this->input->post('bank_detail');
            ...
            $dispatched_throw = $this->input->post('dispatched_throw');
            ...
            $bankdetails = $this->site->getBankDetailByID($bank_id);
            ...
            $data = array('date' => $date,
                ...
                'dispatched_throw' => $dispatched_throw,
            ...
            $coupons = $this->site->getAllAvailableCoupons();
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ") (".$row->sub_name.")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'group' => $customer_group->id);
                } else {
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ") (".$row->sub_name.")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,'group' => $customer_group->id);
                }
            }
            echo json_encode($pr);
        } else {
    ...
    function getGiftCards()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select($this->db->dbprefix('gift_cards') . ".id as id, coupon, card_no, value, balance, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as created_by, customer, expiry", FALSE)
    ...
    function add_gift_card()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $coupon_details = $this->input->post('coupon') ? $this->site->getCouponByID($this->input->post('coupon')) : NULL;
            $coupon = $coupon_details ? $coupon_details->name : NULL;
            
            $data = array('card_no' => $this->input->post('card_no'),
                'value' => $this->input->post('value'),
                'customer_id' => $this->input->post('customer') ? $this->input->post('customer') : NULL,
                'customer' => $customer,
                'coupon_id' => $this->input->post('coupon') ? $this->input->post('coupon') : NULL,
                'coupon' => $coupon,
    ...
    function sale_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('code', 'net_unit_price', 'quantity', 'variant', 'item_tax_rate', 'discount', 'serial', 'hsn_sac', 'per_type');
                $final = array();
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_price']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->sales_model->getProductByCode($csv_pr['code'])) {
                            ...
                            if ($csv_pr['variant']) {
                                ...
                                $item_hsn_sac = $csv_pr['hsn_sac'];
                                $item_per_type = $csv_pr['per_type'];
                                if (isset($item_per_type) && $item_per_type != 0) {
                                $per_type = $this->sales_model->getPerTypeByName($item_per_type);
                                $pr_per_type = $per_type->id;
                                }
...
function Pindex($warehouse_id = NULL)
    {
        $this->load->model('sales_model');
        $this->sma->checkPermissions();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('sales')));
        $meta = array('page_title' => lang('sales'), 'bc' => $bc);
        $this->page_construct('sales/payment_index', $meta, $this->data);
    }

    function getPSales($warehouse_id = NULL)
    {
        $this->load->model('sales_model');
        $this->sma->checkPermissions('index');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("id, date, reference_no, biller, customer, grand_total, paid, (grand_total-paid) as balance, payment_status, due_date")
                ->from('sales')
                ->where('warehouse_id', $warehouse_id);
        } else {
            $this->datatables
