...
class Services extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        ...
        if ($this->Owner || $this->Admin) {
            ...
            $this->data['categories'] = $this->site->getAllCategoriesOnlyService();
        ...
        $this->page_construct('products/service', $meta, $this->data);
    }

    function getServices($warehouse_id = NULL)
    {
        ...
        $detail_link = anchor('products/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('service_details'));
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            <li>' . $detail_link . '</li>
            <li><a href="' . site_url('products/add/$1') . '"><i class="fa fa-plus-square"></i> ' . lang('duplicate_service') . '</a></li>
            <li><a href="' . site_url('products/edit/$1') . '"><i class="fa fa-edit"></i> ' . lang('edit_service') . '</a></li>';
        if ($warehouse_id) {
    ...
    function add($id = NULL)
    {
        ...
        $this->form_validation->set_rules('code', lang("product_code"), 'trim');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $product_code_id = $this->db->query("SELECT MAX( id ) as id FROM ".$this->db->dbprefix('products')." ")->row();
            $product_code_id = $product_code_id->id+1;
            $this->data['category'] = $this->site->getCategoryByID($this->input->post('category'));
        
            $data = array(
                'code' => substr($this->data['category']->name, 0, 1).'10'.$product_code_id,
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('add_service')));
            $meta = array('page_title' => lang('add_service'), 'bc' => $bc);
            $this->page_construct('products/add_service', $meta, $this->data);
        }
    }
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)) {
            ...
            $this->page_construct('products/edit_service', $meta, $this->data);
        }
    }
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('code', 'name', 'category_code', 'unit', 'cost', 'price', 'alert_quantity','hsn_sac','per_type', 'tax_rate', 'tax_method', 'subcategory_code', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6');
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if ($catd = $this->products_model->getCategoryByCode(trim($csv_pr['category_code']))) {
                        ...
                        $pr_hsn[] = trim($csv_pr['hsn_sac']);
                        $per_type = $this->products_model->getPerTypeByName(trim($csv_pr['per_type']));
                        $pr_per[] = $per_type ? $per_type->id : NULL;
                        $cf1[] = trim($csv_pr['cf1']);
                        $cf2[] = trim($csv_pr['cf2']);
                        $cf3[] = trim($csv_pr['cf3']);
                        $cf4[] = trim($csv_pr['cf4']);
            ...
            $ikeys = array('code', 'name', 'category_id', 'unit', 'cost', 'price', 'alert_quantity','hsn_sac','per_type', 'tax_rate', 'tax_method', 'subcategory_id', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6');

            $items = array();
            foreach (array_map(null, $pr_code, $pr_name, $pr_cat, $pr_unit, $pr_cost, $pr_price, $pr_aq,$pr_hsn, $pr_per, $pr_tax, $tax_method, $pr_subcat, $pr_variants, $cf1, $cf2, $cf3, $cf4, $cf5, $cf6) as $ikey => $value) {
    ...
    function modal_view($id = NULL)
    {
        ...
        $this->data['per_types'] = $pr_details->per_type ? $this->site->getPerTypeByID($pr_details->per_type) : NULL;
        ...
        $this->load->view($this->theme.'products/modal_view_service', $this->data);
    }
    ...
    function view($id = NULL)
    {
        ...
        $this->page_construct('products/view_service', $meta, $this->data);
    }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('hsn_sac'));
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('cost'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('price'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('per_types'));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('M1', lang('product_variants'));
                    $this->excel->getActiveSheet()->SetCellValue('N1', lang('pcf1'));
                    $this->excel->getActiveSheet()->SetCellValue('O1', lang('pcf2'));
                    $this->excel->getActiveSheet()->SetCellValue('P1', lang('pcf3'));
                    $this->excel->getActiveSheet()->SetCellValue('Q1', lang('pcf4'));
                    $this->excel->getActiveSheet()->SetCellValue('R1', lang('pcf5'));
                    $this->excel->getActiveSheet()->SetCellValue('S1', lang('pcf6'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $product->hsn_sac);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $product->cost);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $product->price);
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $product->per_type_name);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('L' . $row, $product->subcategory_code);
                        $this->excel->getActiveSheet()->SetCellValue('M' . $row, $product_variants);
                        $this->excel->getActiveSheet()->SetCellValue('N' . $row, $product->cf1);
                        $this->excel->getActiveSheet()->SetCellValue('O' . $row, $product->cf2);
                        $this->excel->getActiveSheet()->SetCellValue('P' . $row, $product->cf3);
                        $this->excel->getActiveSheet()->SetCellValue('Q' . $row, $product->cf4);
                        $this->excel->getActiveSheet()->SetCellValue('R' . $row, $product->cf5);
                        $this->excel->getActiveSheet()->SetCellValue('S' . $row, $product->cf6);
                        $row++;
                    }
    ...
    function getHSN_SAC()
    {
        ...
        $data = $this->products_model->getHSN_SAC($searchTerm);
        //return json data
