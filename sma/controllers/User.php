...
class User extends MY_UserController
{
    ...
    function login($m = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->user_identity($this->input->post('identity'))) {
                ...
                // }
                $identity = $this->session->userdata('bidentity');
                $otp = $this->session->userdata('botp');
                $id = $this->session->userdata('buser_id');
    ...
    function user_login($m = NULL)
    {
        $this->data['title'] = lang('login');

        $bidentity = $this->session->userdata('bidentity');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($this->ion_auth->user_login($bidentity, $this->input->post('password'))) {
                // if ($this->Settings->mmode) {
                //     if (!$this->ion_auth->in_group('owner')) {
                //         $this->session->set_flashdata('error', lang('site_is_offline_plz_try_later'));
                //         redirect('auth/logout');
                ...
                // }
                $this->session->set_flashdata('message', $this->ion_auth->messages());
                $referrer = $this->session->userdata('requested_page') ? $this->session->userdata('requested_page') : 'dashboard';
                redirect($referrer);
            } else {
                $this->session->set_flashdata('error', $this->ion_auth->errors());
                redirect('user/user_login');
            ...
            $this->data['identity'] = array('name' => 'identity',
                ...
                'placeholder' => lang('phone'),
                'readonly' => 'true',
                'value' => $this->form_validation->set_value('identity',$bidentity),
            );
            ...
            $this->load->view($this->theme . 'auth/user_login', $this->data);
        }
        }else{
    ...
    function logout($m = NULL)
    {
        ...
        $logout = $this->ion_auth->user_logout();
        $this->session->set_flashdata('message', $this->ion_auth->messages());
