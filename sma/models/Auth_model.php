...
class Auth_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->user_identity_column = $this->config->item('user_identity', 'ion_auth');
    ...
    public function user_identity($identity)
    {
        echo $identity;
        ...
        $query = $this->db->select($this->user_identity_column . ', name, email, id, active, last_login, last_ip_address, group_id')
            ->where($this->user_identity_column, $this->db->escape_str($identity))
            ->where('group_id', 3)
            ->limit(1)
        ...
        if ($query->num_rows() === 1) {
            ...
            $otp = rand(1111,9999);
            $message="Dear ".$user->name.", your OTP is ".$otp;

            $this->sma->send_sms($identity, $message);

            $otp_data = array('otp' => $otp);
            $this->db->where('id', $user->id);
            $this->db->where('phone', $user->phone);
            $this->db->update('companies', $otp_data);
            ...
            $session_data = array(
            'bidentity' => $user->{$this->user_identity_column},
            'buser_id' => $user->id, //everyone likes to overwrite id so we'll use user_id
            'botp' => $otp,
            );
            ...
            $this->set_message('please_check_your_mobile_no_for_otp');
        ...
        $this->trigger_events('post_login_unsuccessful');
        $this->set_error('login_unsuccessful_invalid');

        return FALSE;
    }

    public function user_login($identity, $password)
    {
        $this->trigger_events('pre_login');
    ...
    public function update_user_last_login($id)
    {
        ...
        $this->db->update($this->tables['companies'], array('last_login' => time()), array('id' => $id));

        return $this->db->affected_rows() == 1;
    }

    public function update_user_last_login_ip($id)
    {
        $this->trigger_events('update_last_login_ip');

        $this->trigger_events('extra_where');

        $this->db->update($this->tables['companies'], array('last_ip_address' => $this->input->ip_address()), array('id' => $id));
    ...
    public function set_user_session($user)
    {
        ...
        $session_data = array(
            'user_identity' => $user->{$this->user_identity_column},
            'user_name' => $user->name,
            'user_email' => $user->email,
            'user' => $user->id, //everyone likes to overwrite id so we'll use user_id
            'user_old_last_login' => $user->last_login,
            'user_last_ip' => $user->last_ip_address,
