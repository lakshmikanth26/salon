...
class Sales_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id, products.code, products.name,subcategories.name as sub_name, type,hsn_sac, per_type, warehouses_products.quantity, price,member_price, tax_rate, tax_method')
            ->join('warehouses_products', 'warehouses_products.product_id=products.id', 'left')
            ->join('subcategories', 'subcategories.id=products.subcategory_id', 'left')
            ->group_by('products.id');
        $this->db->where('products.old_products', '0');
    ...
    {
        $this->db->select('sale_items.*, per_types.name as per_type_name, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant')
            ->join('products', 'products.id=sale_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=sale_items.tax_rate_id', 'left')
            ->join('per_types', 'per_types.id=sale_items.per_type', 'left')
    ...
    public function deleteSale($id)
    {
        ...
        $this->db->delete('sale_staffs', array('sale_id' => $id)) &&
