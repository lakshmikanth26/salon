...
class Site extends CI_Model
{
    ...
    public function get_appointment_alerts() {
        $date = date('Y-m-d');
        $this->db->select('count(id) as alert_num')->where('booked_date =', $date);
    ...
    public function getCompanyByMobileNo($phone) {
        $q = $this->db->get_where('companies', array('phone' => $phone, 'customer_group_id' => 4), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getAllCategoriesOnlyProduct() {
        $this->db->order_by('name');
    ...
    public function getAllCategoriesOnlyService() {
        $this->db->order_by('name');
        ...
        if ($q->num_rows() > 0) {
            ...
            switch ($field) {
                ...
                case 'pos':
                    ...
                    break;
                case 'posc':
                    $prefix = isset($this->Settings->sales_prefix) ? $this->Settings->sales_prefix . '/POSC' : '';
                    break;
                case 're':
    ...
    public function getAllStaffsAvailable($event_start_date) {
        $query = $this->db->query('SELECT  DISTINCT s.id, s.name
                            FROM    sma_staffs s
                            WHERE   NOT EXISTS
                                    (
                                        SELECT  1
                                        FROM    sma_staffs_unavailable_dates a
                                        WHERE   a.staff_id = s.id AND
                                                unavailable_date = "'.$event_start_date.'"
                                    )
                        ');
    ...
    public function getAllWebsiteServices() {
        $this->db->order_by('name');
        $q = $this->db->get('website_services');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getServiceByID($id) {
        $q = $this->db->get_where('website_services', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getStaffID($id) {
        $this->db->select('s.*,w.name as warehouse_name');
        $this->db->from('staffs as s');
        $this->db->join('warehouses as w','s.warehouse_id=w.id','INNER');
    ...
    public function get_staff($warehouse_id) {
        $q = $this->db->get_where('staffs', array('warehouse_id' => $warehouse_id));
        if ($q->num_rows() > 0) {
            $query['response'] = 200;
            $query['data'] = $q->result();
            return $query;
    ...
    public function updateSms($field,$sms_body,$field_sent,$sms_sent) {
        $q = $this->db->get_where('sms_templates', array('id' => '1'), 1);
        if ($q->num_rows() > 0) {
            $ref = $q->row();
            $this->db->update('sms_templates', array($field => $sms_body, $field_sent => $sms_sent), array('id' => '1'));
            return TRUE;
        }
        return FALSE;
    }
    ...
    public function getSalePending() {
    ...
    public function getAllAvailableCoupons() {
        $today = date('Y-m-d');
        $q = $this->db->get_where('coupons', array('start_date <=' => $today, 'expiry_date >=' => $today));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
