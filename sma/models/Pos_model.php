...

{
...

    {
        ...
        $this->db->select('products.id, products.code, products.name,subcategories.name as sub_name, type, warehouses_products.quantity, price,member_price, tax_rate, tax_method')
        ...
        $q = $this->db->get_where("products", array($this->db->dbprefix('products').'.code' => $code, $this->db->dbprefix('products').'.old_products' => 0));
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL, $pinvoice_type = NULL, $staffs = NULL)

    {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            $this->site->updateReference($pinvoice_type);
            ...
            if($staffs){
                ...
                $staff['sale_id'] = $sale_id;  

                $this->db->insert('sale_staffs', $staff);
                ...
                foreach ($payments as $payment) {
                    ...
                    if (!empty($payment) && isset($payment['amount']) && $payment['amount'] != 0) {
                        ...
                        if ($payment['paid_by'] == 'ppp') {
                            ...
                            }elseif($payment['paid_by'] == 'Wallet'){
                                
                                $this->sma->deduct_award_points($payment['amount'], $data['customer_id'], $data['created_by']);
    ...
    public function updateCustomerGroupToMember($customer_id) {
        $data = array(
            'customer_group_name' => 'MEMBER',
            'customer_group_id' => '5'
        );
    ...
    {   
        $this->db->select('award_points as balance');
        $this->db->where('id', $customer_id);
